...
class PanopticNarrativeGroundingDataset(Dataset):
    ...
    def __init__(self, cfg, split, train=True):
        ...
        self.boxes = {}
        self.mask_features_path = osp.join(cfg.DATA.PATH_TO_FEATURES_DIR, split, 'mask_features')
        self.sem_seg_features_path = osp.join(cfg.DATA.PATH_TO_FEATURES_DIR, split, 'sem_seg_features')
        self.panoptic_pred_path = osp.join(cfg.DATA.PATH_TO_FEATURES_DIR, split, 'panoptic_seg_predictions')
        ...
        self.panoptic = self.load_json(osp.join(self.ann_dir, 'panoptic_{:s}.json'.format(split)))
        ...
        if not osp.exists(osp.join(self.ann_dir, 'localized_narratives_{:s}{:s}.json'.format(split, cfg.DATA.DATASET_SUFFIX))):
            print("No such a dataset")
        else:
            self.localized_narratives = self.load_json(osp.join(self.ann_dir, 'localized_narratives_{:s}{:s}.json'.format(split, cfg.DATA.DATASET_SUFFIX)))
        self.localized_narratives = [ln for ln in self.localized_narratives if (torch.tensor([item for sublist in ln['labels'] for item in sublist]) != -2).any()]
    ...
    def __len__(self):
        return len(self.localized_narratives)
    ...
    def __getitem__(self, idx):
        ...
        localized_narrative = self.localized_narratives[idx]
        ...
        img_mask_features = torch.load(osp.join(self.mask_features_path, "{}.pth".format(image_id)), map_location=torch.device('cpu'))
        num_instances = img_mask_features.shape[0]
        if num_instances > 0:
            mask_features[:num_instances, :] = img_mask_features.view(num_instances, -1)

        img_sem_seg_features = torch.load(osp.join(self.sem_seg_features_path, "{}.pth".format(image_id)), map_location=torch.device('cpu'))
        num_labels = img_sem_seg_features.shape[0]
        if num_labels > 0:
            img_sem_seg_features = self.semantic_pool(img_sem_seg_features)
            mask_features[num_instances: num_instances + num_labels, :] = img_sem_seg_features.view(num_labels, -1)

        annotations = torch.zeros([self.cfg.MODEL.MAX_SEQUENCE_LENGTH, num_preds]).long()
        ...
        panoptic_pred = io.imread(osp.join(self.panoptic_pred_path, "{:012d}.png".format(image_id)))[:, :, 0]
        panoptic_pred = torch.tensor(panoptic_pred).long()
        masks = torch.zeros((panoptic_pred.max() + 1, panoptic_pred.shape[0], panoptic_pred.shape[1]))
        ...
        noun_phrases = torch.zeros([self.cfg.MODEL.MAX_SEQUENCE_LENGTH, self.cfg.TEST.MAX_NOUN_PHRASES]).float()
        noun_indices = torch.tensor(localized_narrative["noun_vector"])
        noun_phrases[noun_indices.nonzero().squeeze() + 1, noun_indices[noun_indices != 0] - 1] = 1
        ...
        panoptic_segm = io.imread(osp.join(self.ann_dir, 'panoptic_segmentation_{:s}'.format(self.split), '{:012d}.png'.format(image_id)))
        panoptic_segm = panoptic_segm[:, :, 0] + panoptic_segm[:, :, 1] * 256 + panoptic_segm[:, :, 2] * 256**2
        grounding_instances = torch.zeros([self.cfg.MODEL.MAX_SEQUENCE_LENGTH, 256, 256])
        for i, bbox in enumerate(localized_narrative['boxes']):
            for b in bbox:
                if b != [0] * 4:
                    segment_info = [s for s in panoptic_ann['segments_info'] if s['bbox'] == b][0]
                    segment_cat = [c for c in self.panoptic['categories'] if c['id'] == segment_info['category_id']][0]
                    instance = torch.zeros([image_info['height'], image_info['width']])
                    instance[panoptic_segm == segment_info['id']] = 1
                    grounding_instances[i + 1, :] += self.mask_transform(instance.unsqueeze(dim=0)).squeeze()
        ...
        return caption, boxes, instances, mask_features, annotations, ann_types, ann_categories, noun_phrases, grounding_instances, image_info
