...
from .bert_config import BertConfig, VisualConfig
...
class Bert_Cross_Attention(nn.Module):
    def __init__(self, config, ctx_dim=None):
        super().__init__()
...
class CrossEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.bert_layers_num = VISUAL_CONFIG.l_layers
        self.cross_layers_num = VISUAL_CONFIG.x_layers
        self.single_layers_num = VISUAL_CONFIG.r_layers
        self.layer = nn.ModuleList(
            [BertLayer(config) for _ in range(self.bert_layers_num)]
        )
    ...
    def forward(self, lang_feats, lang_attention_mask,
                visn_feats, visn_attention_mask=None, noun_phrases=None):
...
class Cross_Layer(nn.Module):
    def __init__(self, config):
        ...
        self.visual_attention = Bert_Cross_Attention(config)
